...
class SyncTransformer(nn.Module):
    def __init__(self, n_classes, d_model=512):
        ...
        # self.classifier_n = nn.Linear(d_model, n_classes)

    def forward(self, frame_seq, mel_seq):
        B = frame_seq.shape[0]
        vid_embedding = self.vid_prenet(frame_seq.view(B,-1,3,48,96).permute(0,2,3,4,1).contiguous())
        # print('vid embed size ', vid_embedding.size())
        # print('mel seq size ', mel_seq.size())
        aud_embedding = self.aud_prenet(mel_seq)
        # print('aud embed size ', aud_embedding.size())
        vid_embedding = vid_embedding.squeeze(2).squeeze(2)
        # print('vid embed size ', vid_embedding.size())
        aud_embedding = aud_embedding.view(B, self.d_model, -1)  # .squeeze(2)
        # print('aud embed size ', aud_embedding.size())
        ...
        logits_clsf = self.classifier_n(h_pooled)
